...
function render(d){const lang=$('#publicLanguage').value,b=d.business,profile=d.translations.find(x=>x.language_code===lang)||d.translations.find(x=>x.language_code==='en')||b;for(const [id,key] of Object.entries({publicName:'name',publicCategory:'category',publicAddress:'address'}))$('#'+id).textContent=b[key]||'Not provided';for(const [id,key] of Object.entries({publicDescription:'description',publicServices:'service_details',publicPayments:'payment_methods',publicMenu:'menu_details',publicFacilities:'facility_details'}))$('#'+id).textContent=profile[key]||'Not provided in this language';const items=d.phrases.filter(x=>x.target_language===lang),faqs=d.faqs.filter(x=>x.language_code===lang),terms=d.terms.filter(x=>x.language_code===lang);$('#publicPhrases').innerHTML=items.map(x=>'<button class="scenario-step public-phrase" data-id="'+x.id+'" data-category="'+esc(x.category)+'" data-question="'+esc(x.source_text)+'"><strong>'+esc(x.source_text)+'</strong><span>'+esc(x.translated_text)+'</span></button>').join('')||'<div class="empty-state">No approved phrases in this language.</div>';$('#publicFaqs').innerHTML=faqs.map(x=>'<details><summary>'+esc(x.question)+'</summary><p>'+esc(x.answer)+'</p></details>').join('')||'<div class="empty-state">No FAQs in this language.</div>';$('#publicTerms').innerHTML=terms.map(x=>'<div class="record-item"><div><strong>'+esc(x.term)+'</strong><small>'+esc(x.explanation)+'</small></div></div>').join('')||'<div class="empty-state">No terms in this language.</div>';document.querySelectorAll('.public-phrase').forEach(x=>x.onclick=async()=>{const row=items.find(p=>String(p.id)===x.dataset.id);openOverlay(row.translated_text,row.suggested_reply||'','Approved business quick reply');await recordQuestion(x.dataset.category,x.dataset.question);});$('#publicLanguage').onchange=()=>render(d);$('#sendCustomQuestion').onclick=async()=>{const question=$('#publicCustomQuestion').value.trim();if(!question){$('#questionStatus').textContent='Enter a question before sending.';$('#publicCustomQuestion').focus();return;}await recordQuestion('Custom question',question);$('#questionStatus').textContent='Question recorded for the business. Use the approved phrases above for an immediate reply.';$('#publicCustomQuestion').value='';};}
async function recordQuestion(category,question,requiresReply=false){const r=await fetch('api/public_business.php?slug='+encodeURIComponent(slug),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({category,language:$('#publicLanguage').value,question,requires_reply:requiresReply})});return r.ok?await r.json():null;}
function questionStorageKey(){return 'tourlingo_question:'+slug;}
async function checkQuestionReply(){const token=localStorage.getItem(questionStorageKey());if(!token)return;try{const r=await fetch('api/public_business.php?slug='+encodeURIComponent(slug)+'&question_token='+encodeURIComponent(token)),d=await r.json();if(!r.ok)return;const q=d.question;$('#visitorReplyCard').hidden=false;$('#visitorReplyStatus').textContent=q.status==='answered'?'Answered by the business':q.status.replace('_',' ');$('#visitorReplyText').textContent=q.reply_text||'The business has not replied yet. Check again later.';}catch(error){$('#visitorReplyText').textContent='Could not check the reply. Please try again.';}}
$('#sendCustomQuestion').addEventListener('click',async event=>{event.stopImmediatePropagation();const question=$('#publicCustomQuestion').value.trim();if(!question){$('#questionStatus').textContent='Enter a question before sending.';$('#publicCustomQuestion').focus();return;}const result=await recordQuestion('Custom question',question,true);if(!result?.question_token){$('#questionStatus').textContent='The question could not be sent. Please try again.';return;}localStorage.setItem(questionStorageKey(),result.question_token);$('#publicCustomQuestion').value='';$('#questionStatus').textContent='Question sent to the business.';$('#visitorReplyCard').hidden=false;$('#visitorReplyStatus').textContent='New';$('#visitorReplyText').textContent='The business has not replied yet. Check again later.';},true);
