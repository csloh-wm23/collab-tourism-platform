(function(root,factory){
    ...
    function speechBiasPhrases(){
        return ['cappuccino','iced coffee','coffee','Kuala Lumpur','Malaysia','nasi lemak','nasi campur','mamak','surau','tapau','roti canai','teh tarik','halal','vegetarian','train station','bus station','hotel','restaurant','emergency'];
    }
    function normalizeSpeechTranscript(transcript){
        const value=String(transcript||'').replace(/\s+/g,' ').replace(/\s+([,.!?;:])/g,'$1').trim();
        const exact={'capuccino':'cappuccino','cappucino':'cappuccino','tapuccino':'cappuccino','cup of chino':'cappuccino','couple chino':'cappuccino'};
    ...
    function bestSpeechAlternative(result,phrases=speechBiasPhrases()){
        ...
        const score=alternative=>{
            ...
            const bias=vocabulary.reduce((best,phrase)=>Math.max(best,transcript===phrase?2:transcript.includes(phrase)?.35:0),0);
            return (Number(alternative.confidence)||0)+bias;
        };
        const selected=alternatives.reduce((best,item)=>score(item)>score(best)?item:best);
        return {transcript:normalizeSpeechTranscript(selected.transcript),confidence:Number(selected.confidence)||0};
    }
    return {twoWayLanguages,confidenceLabel,normalizedAnalyticsLanguage,speechRecognitionLanguage,recoverableSpeechError,speechBiasPhrases,normalizeSpeechTranscript,bestSpeechAlternative};
}));
