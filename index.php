        ...
        <div class="content-wrap">
            ...
            <section class="page active" id="home" data-title="Overview">
                ...
                <section class="getting-started" aria-labelledby="gettingStartedTitle">
                    <div><span class="eyebrow">Recommended next steps</span><h2 id="gettingStartedTitle">Get the most from TourLingo</h2><p>A short path from setup to confident communication.</p></div>
                    <div class="progress-steps">
                        <button type="button" data-page="profile"><span>1</span><strong>Complete your profile</strong><small>Add your language and preferences</small></button>
                        <button type="button" data-page="assistance"><span>2</span><strong>Build a travel guide</strong><small>Prepare phrases for a situation</small></button>
                        <button type="button" data-page="communication"><span>3</span><strong>Try a translation</strong><small>Type or speak a message</small></button>
                    </div>
                </section>
...
<?php endif; ?>
    ...
    : 'Under review' ?></span></div><div class="narrow-content"><article class="card-panel application-card"><div class="card-kicker"><span>⌂</span><div><h2>Business details</h2><p>Keep this information accurate while our team reviews your application.</p></div></div><label>Business name<input id="applicationName" maxlength="160"></label><label>Category<input id="applicationCategory" maxlength="80"></label><label>Address<textarea id="applicationAddress" maxlength="500" rows="4"></textarea></label><button id="resubmitApplication" class="primary">Save and submit for review</button></article></div></section>
            ...
            <section class="page" id="business" data-title="Business studio">
                ...
                <div class="publish-progress" aria-label="Business profile publishing steps"><span class="done"><b>✓</b><small>Business approved</small></span><i></i><span class="active"><b>2</b><small>Complete profile</small></span><i></i><span><b>3</b><small>Add visitor content</small></span><i></i><span><b>4</b><small>Preview and share</small></span></div>
                <div class="module-tabs" role="tablist" aria-label="Business studio sections"><button class="active" type="button" data-business-tab="profile">Public profile</button><button type="button" data-business-tab="content">Content studio</button><button type="button" data-business-tab="library">Content library</button><button type="button" data-business-tab="inbox">Question inbox <span id="businessInboxCount" class="tab-count">0</span></button><button type="button" data-business-tab="reports">Visitor reports</button></div>
                <div class="module-view active" data-business-view="profile"><div class="business-profile-grid">
                    <article class="card-panel"><div class="section-heading"><h2>Public service profile</h2><p>Choose a language and keep your visitor information clear.</p></div><label>Content language<select id="businessContentLanguage"><?= language_options() ?></select></label><div class="form-grid two-columns"><label>Name<input id="businessName" maxlength="160"></label><label>Category<input id="businessCategory" maxlength="80"></label></div><label>Address<textarea id="businessAddress" maxlength="500"></textarea></label><label>Description<textarea id="businessDescription" maxlength="3000"></textarea></label><label>Services<textarea id="businessServices" maxlength="3000"></textarea></label><div class="form-grid two-columns"><label>Payment methods<input id="businessPayments" maxlength="500"></label><label>Menu explanations<textarea id="businessMenu" maxlength="5000"></textarea></label></div><label>Facility explanations<textarea id="businessFacilities" maxlength="3000"></textarea></label><label class="switch-row"><span><strong>Public tourist page</strong><small>Allow travellers to open your shared business page.</small></span><input type="checkbox" id="businessPublic"></label><button id="saveBusiness" class="primary">Save</button></article>
                    <aside class="share-card"><span class="eyebrow light-eyebrow">Live traveller preview</span><h2>See what visitors see</h2><div class="phone-preview"><div class="phone-speaker"></div><div class="phone-preview-brand">TourLingo partner</div><strong id="businessPreviewName">Your business</strong><span id="businessPreviewCategory">Tourism service</span><div class="phone-preview-card"><small>Service information</small><p id="businessPreviewDescription">Add a description to preview your public page.</p></div></div><p>Scan the QR code or open the public page for the full visitor experience.</p><div id="businessQr" class="qr-box"></div><small id="qrStatus"></small><a id="publicBusinessLink" class="secondary full-button" target="_blank" rel="noopener">Open tourist page</a></aside>
                </div></div>
                <div class="module-view" data-business-view="content"><div class="content-studio-grid">
                    <article class="card-panel studio-card"><h2>Approved phrase</h2><p>Add a phrase travellers can use immediately.</p><label>Question or phrase<input id="phraseSource" maxlength="500"></label><label>Translation<input id="phraseTranslated" maxlength="500"></label><label>Suggested reply<input id="phraseReply" maxlength="500"></label><div class="form-grid two-columns"><label>Target language<select id="phraseTarget"><?= language_options() ?></select></label><label>Category<input id="phraseCategory" maxlength="80" value="General"></label></div><button id="addBusinessPhrase" class="primary">Add phrase</button></article>
                    <article class="card-panel studio-card"><h2>Frequently asked question</h2><p>Answer a common question once and publish it clearly.</p><label>Language<select id="faqLanguage"><?= language_options() ?></select></label><label>Question<input id="faqQuestion" maxlength="500"></label><label>Answer<textarea id="faqAnswer" maxlength="1000"></textarea></label><button id="addFaq" class="primary">Add FAQ</button></article>
                    <article class="card-panel studio-card"><h2>Local terminology</h2><p>Explain a Malaysian word or expression for visitors.</p><label>Language<select id="termLanguage"><?= language_options() ?></select></label><label>Term<input id="businessTerm" maxlength="120"></label><label>Explanation<input id="businessTermExplanation" maxlength="500"></label><button id="addBusinessTerm" class="primary">Save term</button></article>
                </div></div>
                ...
                <div class="module-view" data-business-view="inbox"><article class="card-panel"><div class="card-title-row"><div><span class="eyebrow">Visitor questions</span><h2>Question inbox</h2><p>Track every question from new to answered.</p></div><button id="refreshBusinessInbox" class="secondary" type="button">Refresh</button></div><div class="inbox-filters" role="group" aria-label="Filter questions"><button class="chip active" type="button" data-q-filter="all">All</button><button class="chip" type="button" data-q-filter="new">New</button><button class="chip" type="button" data-q-filter="in_progress">In progress</button><button class="chip" type="button" data-q-filter="answered">Answered</button><button class="chip" type="button" data-q-filter="archived">Archived</button></div><div id="businessInbox" class="business-inbox"></div></article></div>
            ...
            <section class="page" id="insights" data-title="Insights">
                ...
                <details class="card-panel report-review-panel mt-large" aria-labelledby="reportReviewTitle">
                    <summary id="reportReviewTitle" class="report-review-summary">
                        <span class="report-review-icon" aria-hidden="true"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.5 8.5 0 0 1-8.5 8.5H4l-3 2V11.5a8.5 8.5 0 0 1 17-1" transform="translate(1 -1) scale(.95)"/><path d="M8 8h7M8 12h5"/></svg></span>
                        <span class="report-review-heading"><strong>Translation reports</strong><span>Review unclear translations and track follow-up actions.</span></span>
                        <svg class="report-review-chevron" aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 5 7 7-7 7"/></svg>
                    </summary>
