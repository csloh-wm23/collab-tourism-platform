...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf($_POST['csrf'] ?? null)) {
        ...
        ($values['business_name'] === '' || $values['address'] === '')
    ) {
        ...
        try {
            ...
            if ($values['role'] === 'tourist') {
                $db->prepare('INSERT INTO tourist_profiles (user_id) VALUES (?)')->execute([
                    $userId,
                ]);
            } else {
                $slugBase = strtolower(
                    trim(preg_replace('/[^a-z0-9]+/i', '-', $values['business_name']), '-'),
                );
                $slug = ($slugBase ?: 'business') . '-' . $userId;
                $db->prepare(
                    'INSERT INTO businesses (owner_user_id, name, category, address, verification_status, qr_slug) VALUES (?, ?, ?, ?, ?, ?)',
                )->execute([
                    $userId,
                    mb_substr($values['business_name'], 0, 160),
                    mb_substr($values['category'], 0, 80),
                    mb_substr($values['address'], 0, 500),
                    'pending',
                    $slug,
                ]);
            }
            ...
            $error =
                $exception->getCode() === '23000'
                    ...
                    : 'Registration failed. Check the database and try again.';
        }
    }
}
?>
    ...
    $error,
            ...
            <form method="post">
                ...
                <div class="form-grid two-columns">
                    <label>Full name<input name="full_name" autocomplete="name" required value="<?= htmlspecialchars(
                        $values['full_name'],
                    ) ?>" placeholder="Your name"></label>
                    <label>Email address<input name="email" type="email" autocomplete="email" required value="<?= htmlspecialchars(
                        $values['email'],
                    ) ?>" placeholder="you@example.com"></label>
                </div>
                <label>Account type
                    <select name="role" id="roleSelect">
                        <option value="tourist" <?= $values['role'] === 'tourist'
                            ? 'selected'
                            : '' ?>>Traveller</option>
                        <option value="business" <?= $values['role'] === 'business'
                            ? 'selected'
                            : '' ?>>Tourism business</option>
                    </select>
                </label>
                ...
                <div id="businessFields">
                    ...
                    <label>Business name<input name="business_name" value="<?= htmlspecialchars(
                        $values['business_name'],
                    ) ?>"></label>
                    <div class="form-grid two-columns"><label>Category<select name="category"><option>Food & drink</option><option>Accommodation</option><option>Attraction</option><option>Transport</option><option>Tour operator</option></select></label><label>Address<textarea name="address" rows="2"><?= htmlspecialchars(
                        $values['address'],
                    ) ?></textarea></label></div>
                </div>
                <div class="form-grid two-columns">
