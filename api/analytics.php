...
try{
 $user=current_user();$consented=false;
 if($user&&($user['role']??'')==='tourist'&&($user['status']??'')==='active'){$consent=database()->prepare("SELECT is_granted FROM consent_records WHERE user_id=? AND consent_type='anonymous_analytics' ORDER BY recorded_at DESC,id DESC LIMIT 1");$consent->execute([(int)$user['id']]);$consented=(int)$consent->fetchColumn()===1;}
 else{$consented=($_SESSION['guest_preferences']['analytics']??false)===true;}
 if(!$consented)analytics_reply(['ok'=>true,'recorded'=>false]);
 $event=clean_text($input['event_type']??'',80,true);$rawLanguage=$input['language_code']??null;$language=($rawLanguage===null||$rawLanguage===''||$rawLanguage==='auto')?null:clean_language($rawLanguage);
