...
try {
    ...
    if (!$registeredTourist) {
        if ($method === 'GET') {
            preferences_reply([
                'ok' => true,
                'preferences' => $_SESSION['guest_preferences'] ?? [
                    'save_history' => true,
                    'analytics' => false,
                ],
            ]);
        ...
        if (!is_array($input) || !verify_csrf($input['csrf'] ?? null)) {
            preferences_reply(['ok' => false, 'message' => 'Invalid request token.'], 403);
        }
        if (
            !isset($input['save_history'], $input['analytics']) ||
            !is_bool($input['save_history']) ||
            !is_bool($input['analytics'])
        ) {
            preferences_reply(
                ['ok' => false, 'message' => 'Both privacy choices must be true or false.'],
                422,
            );
        }
        ...
        preferences_reply([
            'ok' => true,
            'message' => 'Guest privacy choices saved for this session.',
        ]);
    }
    ...
    if ($method === 'GET') {
        ...
        foreach ($stmt->fetchAll() as $row) {
            ...
            $seen[$type] = true;
            if ($type === 'save_translation_history') {
                $preferences['save_history'] = (bool) $row['is_granted'];
            }
            if ($type === 'anonymous_analytics') {
                $preferences['analytics'] = (bool) $row['is_granted'];
            }
        }
    ...
    $input = json_decode((string) file_get_contents('php://input'), true);
    if (!is_array($input)) {
        preferences_reply(['ok' => false, 'message' => 'Invalid JSON body.'], 400);
    }
    if (!verify_csrf($input['csrf'] ?? null)) {
        preferences_reply(['ok' => false, 'message' => 'Invalid request token.'], 403);
    }
    if (
        !array_key_exists('save_history', $input) ||
        !is_bool($input['save_history']) ||
        !array_key_exists('analytics', $input) ||
        !is_bool($input['analytics'])
    ) {
        preferences_reply(
            ['ok' => false, 'message' => 'Both privacy choices must be true or false.'],
    ...
    $db->beginTransaction();
    $insert = $db->prepare(
        'INSERT INTO consent_records (user_id,consent_type,is_granted,policy_version) VALUES (?,?,?,?)',
    );
    $insert->execute([
        $userId,
        'save_translation_history',
        $input['save_history'] ? 1 : 0,
        '2026-08',
    ]);
