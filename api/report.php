...
function report_reply(array $body, int $status = 200): never
{
...
if (!is_array($input)) {
    report_reply(['ok' => false, 'message' => 'Invalid JSON body.'], 400);
}
if (!verify_csrf($input['csrf'] ?? null)) {
    report_reply(['ok' => false, 'message' => 'Invalid request token.'], 403);
}
try {
    $source = clean_text($input['source_text'] ?? '', 1000, true);
    $translated = clean_text($input['translated_text'] ?? '', 1000, true);
    $from = clean_language($input['source_language'] ?? '', true);
    $to = clean_language($input['target_language'] ?? '');
    ...
    report_reply(['ok' => false, 'message' => $e->getMessage()], 422);
} catch (Throwable $e) {
    error_log('Report: ' . $e->getMessage());
    report_reply(['ok' => false, 'message' => 'Could not submit the report.'], 500);
}
