...
try{
 ...
 $issue=clean_text($input['issue_type']??'unclear_translation',80,true);$notes=clean_text($input['notes']??'',1000);
 $user=current_user();$stmt=database()->prepare('INSERT INTO translation_reports(reporter_user_id,source_text,translated_text,source_language,target_language,scenario,confidence,issue_type,notes) VALUES(?,?,?,?,?,?,?,?,?)');
 $stmt->execute([$user?(int)$user['id']:null,$source,$translated,$from,$to,$scenario,$confidence,$issue,$notes]);
